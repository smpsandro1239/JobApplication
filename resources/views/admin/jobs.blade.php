              ...
              <div class="card">
                <div class="card-body">
                  <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="card-title mb-0">Jobs Table</h4>
                    <button type="button" class="btn btn-sm btn-primary" data-toggle="modal"
                      data-target="#addJobModal">
                      <i class="fas fa-user-plus"></i> Add Job
                    </button>
                  </div>
                  @if(session('success'))
                        ...
                        <div class="modal-body">
                          <form action="{{ route('admin.jobs.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                              ...
                              <textarea class="form-control" id="job_summary" name="job_summary" rows="3" required></textarea>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                              <label for="region_or_remote">Region or Remote</label>
                              <input type="text" class="form-control" id="region_or_remote" name="region_or_remote" required>
                            </div>
                            <div class="form-group">
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped">
                      ...
                      <tbody>
                        @foreach($jobs as $job)
                        <tr>
                          <td> @if($job->image) <!-- Check if image exists -->
                            <img src="{{ asset('storage/' . $job->image) }}" alt="{{ $job->title }}" style="width: 50px; height: auto;"> <!-- Adjust width/height as needed -->
                            @else
                            No Image <!-- Placeholder if no image is available -->
                            @endif
                          </td>
                          <td>{{ $job->title }}</td>
                          <td>{{ $job->company_name }}</td>
                          <td>
                            {{ \Illuminate\Support\Str::words($job->job_summary, 2, '...') }}
                            <a href="#" data-toggle="modal" data-target="#jobSummaryModal-{{ $job->id }}">Read More</a>
                          </td>
                        ...
                        <!-- Job Summary Modal -->
                        <div class="modal fade" id="jobSummaryModal-{{ $job->id }}" tabindex="-1" role="dialog" aria-labelledby="jobSummaryModalLabel-{{ $job->id }}" aria-hidden="true">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="jobSummaryModalLabel-{{ $job->id }}">Full Job Summary</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                                </button>
                              </div>
